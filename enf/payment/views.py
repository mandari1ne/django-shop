...
def create_stripe_checkout_session(order, request):
    ...
    try:
        checkout_session = stripe.checkout.Session.create(
            ...
            cancle_url=request.build_absolute_uri('/payment/stripe/cancel') + f'order_id={order.id}',
            metadata={
                'order_id': order.id
            }
        )
...
@csrf_exempt
# решает толбко post запросы
@require_POST
# webhook принимает только запросы от stripe
def stripe_webhook(request):
    ...
    except stripe.errorr.SignatureVerificationError as e:
        return HttpResponse(status=400)
